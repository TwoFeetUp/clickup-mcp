...
import { getApiResources, readApiResource } from "./tools/api-resources.js";
...
export function configureServer() {
  ...
  // Add handler for resources/list - expose API files for code execution
  server.setRequestHandler(ListResourcesRequestSchema, async (req) => {
    logger.debug("Received ListResources request");
    try {
      const resources = getApiResources();
      logger.info(`Returning ${resources.length} API resources for code execution pattern`);
      return { resources };
    } catch (err) {
      logger.error("Error listing API resources", err);
      return { resources: [] };
    }
  });

  // Add handler for resources/read - read API file content
  server.setRequestHandler(ReadResourceRequestSchema, async (req) => {
    const uri = req.params?.uri;
    logger.debug(`Received ReadResource request for: ${uri}`);

    try {
      const content = readApiResource(uri);
      return {
        contents: [{
          uri,
      ...
      logger.error(`Error reading resource: ${uri}`, err);
